    ...
    public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) {
        if (pageIndex == 0) {
            if (panel != null) {
                ...
                graphics.drawString("Recibo de pago", 25, 30);
                graphics.setFont(normalFont2);
                graphics.drawString("Producto", 25, 60);
                graphics.drawString("Cantidad", 100, 60);
                graphics.drawString("Importe", 180, 60);
                tempoVentas = TempoVentas();
                if (0 < tempoVentas.size()) {
                    tempoVentas.forEach(item -> {
                        graphics.setColor(Color.BLACK);
                        graphics.drawString(item.getDescripcion(), 25, 80);
                        graphics.drawString(String.valueOf(item.getCantidad()), 100, 80);
                        graphics.setColor(new Color(0, 153, 102));
                        graphics.drawString(item.getImporte(), 180, 80);
                        pos += 15;
                        importe += formato.reconstruir(item.getImporte().replace("$", ""));
                    });
                    pos += 15;
                    graphics.setColor(Color.BLACK);
                    graphics.drawString("Total: ", 25, 100);
                    graphics.drawString("$" + formato.decimal(importe), 120, 100);
                    pos += 15;
//                   
                }
                graphics.drawString("Papelería Manantial", 110, 130);
                importe = 0;
            }
